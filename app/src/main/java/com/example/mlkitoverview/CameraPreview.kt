        ...
        Row(
            ...
            Button(
                onClick = {
                    ...
                    startRecording(
                        context, videoCapture, executer, { uri ->
                            onImageCaptured(uri)
...
fun startRecording(
    ...
    val contentValues = ContentValues().apply {
        put(MediaStore.MediaColumns.DISPLAY_NAME, "${System.currentTimeMillis()}BySamVideo.mp4")
    ...
    val recordingListener = Consumer<VideoRecordEvent> { event ->
        when (event) {
            is VideoRecordEvent.Start -> {
                Log.d("cameraTAG", "Video recording started")
            }

            is VideoRecordEvent.Finalize -> {
                val savedUri = event.outputResults.outputUri
                Log.d("cameraTAG", "Video recording finalized: $savedUri")
                onVideoCaptured(savedUri)
            }

            is VideoRecordEvent.Pause -> {
                Log.d("cameraTAG", "Video recording paused")
            }

            is VideoRecordEvent.Resume -> {
                Log.d("cameraTAG", "Video recording resumed")
            }
        }
    }
        ...
        ContextCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED)
    {
        videoCapture?.output
            ...
            ?.withAudioEnabled()
            ?.start(executer, recordingListener)
